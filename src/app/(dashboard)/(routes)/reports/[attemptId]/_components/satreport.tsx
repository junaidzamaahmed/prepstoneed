...
export default function Satreport({
  ...
  let score = 0;
  attempt?.quiz.sections.forEach((section: any) => {
  ...
  const attSecs =
    attempt?.quiz.category?.name == "DU"
      ...
      : attempt?.quiz.sections.filter((section: any) =>
  ...
  attSecs[0]?.questions.forEach((question: any) => {
    question.responses.forEach((response: any) => {
      if (response.isCorrect) {
        eng1Score++;
      }
  ...
  attSecs[1]?.questions.forEach((question: any) => {
    question.responses.forEach((response: any) => {
      if (response.isCorrect) {
        eng2Score++;
      }
  ...
  return (
    <div className="mx-auto p-4">
      ...
      {attempt?.quiz.sections.map((section: any) => {
        return (
          ((attempt?.quiz?.category?.name != "DU" &&
            attempt.sections.includes(section.id)) ||
            attempt?.quiz?.category?.name == "DU") && (
                      ...
                      <div className="p-2 bg-white flex">
                        {question.answers.find(
                          ...
                          <div>
                            ...
                            (response: any) => response.attemptID === attempt.id
                          )?.inputText ? (
