...
import { PracticeTestRelations, Quiz } from "@prisma/client";
import axios from "axios";
import { toast } from "sonner";

export default function SelectPracticeTests({
  initialData,
  courseId,
  tests,
}: {
  ...
  const [selected, setSelected] = React.useState<Quiz[]>(
    initialData.practiceTestRelations.map(
      (r: PracticeTestRelations & { quiz: Quiz }) => r.quiz
    )
  );
  ...
  const handleSelect = React.useCallback(async (test: Quiz) => {
    try {
      setDisabled(true);
      await axios.post(`/api/practice-tests/`, { courseId, quizId: test.id });
  ...
  const handleUnselect = React.useCallback(async (test: Quiz) => {
    try {
      setDisabled(true);
      await axios.delete(`/api/practice-tests/`, {
        data: { courseId, quizId: test.id },
      });
